...
sub vcl_recv {
	...
	#if (req.http.X-SS-Auth != "<preshared key>" && req.http.X-SS-Auth != "<previous preshared key>" && ! req.http.Authorization ~ "Basic <base64 hash>" ) {
	if (req.http.X-SS-Auth != "<preshared key>" && ! req.http.Authorization ~ "Basic <base64 hash>" ) {
